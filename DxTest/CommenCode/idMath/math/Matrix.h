...
public:
	...
	int				GetDimension( void ) const;
	void Decompose(idVec3& scale, idQuat& rotation, idVec3& position)const;
	

	//transformation order is scale->rotation->translation
	void Compose(const idVec3& scale, const idQuat& rotation, const idVec3& translation);

	//transformation order is scale->rotation->translation
	void Compose(const idQuat& rotation, const idVec3& translation);
	...
	idVec3 Rotate(const idVec3& input)const;

	//-------------------------
	const float *	ToFloatPtr( void ) const;
	float *			ToFloatPtr( void );
	const char *	ToString( int precision = 2 ) const;
	idMat3			ToMatrix3(void)const;

	idVec3			TransformNormal(idVec3& vec);
	idVec3			TransformCoord(idVec3& vec);
...
ID_INLINE void idMat4::ToFloatsRowMajor(float* pOut16)const
{
	for (int i=0; i<4; ++i)
	{
		pOut16[4*i] = mat[i].x;
...
ID_INLINE void idMat4::ToFloatsColumnMajor(float* pOut16)const
{
	for (int i=0; i<4; ++i)
	{
		pOut16[i] = mat[i].x;
...
ID_INLINE void idMat4::FromFloatsRowMajor(const float* pIn16)
{
	for (int i=0; i<4; ++i)
	{
		mat[i].x = pIn16[4*i] ;
...
ID_INLINE void idMat4::FromFloatsColumnMajor(const float* pIn16)
{
	for (int i=0; i<4; ++i)
	{
		mat[i].x = pIn16[i];
...
ID_INLINE idVec3 idMat4::Rotate(const idVec3& input)const
{
	return idVec3( mat[0].x*input.x +mat[0].y*input.y + mat[0].z*input.z,
...
ID_INLINE idVec3 idMat4::TransformNormal(idVec3& vec)
{
	return idVec3(
...
ID_INLINE idVec3 idMat4::TransformCoord(idVec3& vec)
{
	return idVec3(
		mat[ 0 ].x * vec.x + mat[ 0 ].y * vec.y + mat[ 0 ].z * vec.z + mat[ 0 ].w,
		mat[ 1 ].x * vec.x + mat[ 1 ].y * vec.y + mat[ 1 ].z * vec.z + mat[ 1 ].w,
...
ID_INLINE void idMat4::Decompose(idVec3& scale, idQuat& rotation, idVec3& position)const
{
	idMat4 temp = *this;

	

	scale.x = idMath::Sqrt(mat[0].x*mat[0].x+mat[1].x*mat[1].x+mat[2].x*mat[2].x);
	scale.y = idMath::Sqrt(mat[0].y*mat[0].y+mat[1].y*mat[1].y+mat[2].y*mat[2].y);
	scale.z = idMath::Sqrt(mat[0].z*mat[0].z+mat[1].z*mat[1].z+mat[2].z*mat[2].z);
	...
	position.y = temp[1].w;
	position.z = temp[2].w;
	idMat3 rot; //H3DMat3 is column majored!!!!
	...
	idVec3 column0 = rot[0];
	idVec3 column1 = rot[1];
	idVec3 column2 = rot[2];
	idVec3 row0(column0.x, column1.x, column2.x);
	idVec3 row1(column0.y, column1.y, column2.y);
	idVec3 row2(column0.z, column1.z, column2.z);
	...
	idMat3 rot2 = rotation.ToMat3();
	int a = 0;
}

//transformation order is scale->rotation->translation
ID_INLINE void idMat4::Compose(const idVec3& scale, const idQuat& rotation, const idVec3& translation)
{
	idMat3 matScale;
	//matScale.Identity();
	matScale.Scale(scale);
	idMat3 matRot = rotation.ToMat3();
...
//transformation order is scale->rotation->translation
ID_INLINE void idMat4::Compose(const idQuat& rotation, const idVec3& translation)
{
	idMat3 matRot = rotation.ToMat3();
	mat[0].Set(matRot[0].x, matRot[0].y, matRot[0].z, translation.x);
	mat[1].Set(matRot[1].x, matRot[1].y, matRot[1].z, translation.y);
...
ID_INLINE idMat3 idMat4::ToMatrix3(void) const {
	// NOTE: idMat3 is transposed because it is column-major
	idMat3 m3;
	m3[0][0] = mat[ 0 ][ 0 ];
	m3[1][0] = mat[ 0 ][ 1 ];
...
ID_INLINE idMatX::~idMatX( void ) {
	// if not temp memory
	if ( mat != 0 && ( mat < idMatX::tempPtr || mat > idMatX::tempPtr + MATX_MAX_TEMP ) && alloced != -1 ) {
		Mem_Free16( mat );
	}
}
